...
import { generateProductsItem, initialAppState } from './model';
...
describe('withStorageSync', () => {
    ...
    it('should retrieve values from storage during initialization and update the store state based on keys', () => {
        ...
        TestBed.runInInjectionContext(() => {
            const Store = signalStore(withState(initialAppState), withStorageSync(localStorage, [{ 'products': ['items'] }], '', { sync: true }));
    ...
    it('should write to storage when the state changes if sync is enabled', () => {
        TestBed.runInInjectionContext(async () => {
            const Store = signalStore({ protectedState: false }, withState(initialAppState), withStorageSync(localStorage, [{ 'products': ['items'] }], '', { sync: true }));
            const store = new Store();
    ...
    it('should not write to storage when sync is false and the state changes', () => {
        TestBed.runInInjectionContext(async () => {
            const Store = signalStore({ protectedState: false }, withState(initialAppState), withStorageSync(localStorage, [{ 'products': ['items'] }], '', { sync: false }));
            const store = new Store();
            TestBed.flushEffects();
            const newItems = [generateProductsItem()];
            patchState(store, (state) => ({
