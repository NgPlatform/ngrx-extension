 ...
 */
export function withStorageSync(storage, nodes, prefix, config) {
    return signalStoreFeature(withMethods((store) => ({
        ...
        readFromStorage() {
            readDfs(nodes, prefix, (fullKeyPath) => {
                const jsonString = storage.getItem(fullKeyPath);
                if (jsonString === null) {
                    return;
                }
                const slicedKeys = fullKeyPath.split('-').filter((x) => x !== prefix);
 ...
 */
function writeDfs(currentState, nodes, prefix, callback) {
    nodes.forEach((node) => {
        if (typeof node === 'string') {
            ...
            for (const [key, childNode] of Object.entries(node)) {
                const nestedState = currentState[key];
 ...
 */
function readDfs(nodes, prefix, callback) {
    nodes.forEach((node) => {
        if (typeof node === 'string') {
 ...
 */
function createObject(jsonString, nodesPath, nodesIdx, currentState) {
